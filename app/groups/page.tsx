...
export default function GroupsPage() {
  ...
  const handleCreate = async () => {
    ...
    const { data: group, error } = await supabase.from('groups').insert({
      name: newName.trim(), creator_id: userId, invite_code: code, competition: newComp,
    }).select().single();
                      ...
                      {tab === 'manage' && (
                        <div style={{ padding: '16px' }}>
                          ...
                          <div style={{ background: 'rgba(139,92,246,.05)', border: '1px solid rgba(139,92,246,.2)', borderRadius: '10px', padding: '12px', marginBottom: '12px' }}>
                            ...
                            <p style={{ fontSize: '12px', color: '#9CA3AF', margin: '0 0 8px', lineHeight: 1.5 }}>Share your league leaderboard every matchweek.</p>
                            <button onClick={() => {
