...
export default function Profile() {
  ...
  const [isFoundingForecaster, setIsFoundingForecaster] = useState(false);
  ...
  useEffect(() => {
    const getProfile = async () => {
      try {
        ...
        // ── Check Founding Forecaster ──
        const hasFoundingBadge = badgeData?.some(
          (b: any) => b.badge_type === 'founding_forecaster'
        );
        setIsFoundingForecaster(hasFoundingBadge ?? false);
  ...
  return (
    <main style={{ backgroundColor: '#0D1F0F', minHeight: '100vh', fontFamily: 'Arial, sans-serif', color: 'white' }}>
      ...
      <section style={{ textAlign: 'center', padding: '50px 20px 32px', background: 'linear-gradient(180deg, #0D2B14 0%, #0D1F0F 100%)' }}>
        ...
        {/* ── Founding Forecaster Badge ── */}
        {isFoundingForecaster && (
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', backgroundColor: 'rgba(245,158,11,0.1)', border: '1px solid #F59E0B', borderRadius: '999px', padding: '4px 16px', marginBottom: '8px' }}>
            <span style={{ fontSize: '14px' }}>🏅</span>
            <span style={{ fontSize: '12px', color: '#F59E0B', fontWeight: 'bold', letterSpacing: '0.5px' }}>Founding Forecaster</span>
            <span style={{ fontSize: '10px', color: '#6B7280' }}>· Est. before World Cup 2026</span>
          </div>
        )}
        ...
        {badges.filter(b => b.badge_type !== 'founding_forecaster').length > 0 && (
          <div style={{ display: 'inline-block', backgroundColor: '#1C1A3A', border: '1px solid #3B82F6', borderRadius: '999px', padding: '4px 16px', marginBottom: '12px', marginLeft: '8px' }}>
            <span style={{ fontSize: '13px', color: '#93C5FD', fontWeight: 'bold' }}>
              🏅 {badges.filter(b => b.badge_type !== 'founding_forecaster').length} badge{badges.filter(b => b.badge_type !== 'founding_forecaster').length !== 1 ? 's' : ''}
            </span>
          </div>
        )}
        <br />
      ...
      <section style={{ maxWidth: '600px', margin: '0 auto', padding: '0 20px 24px' }}>
        ...
        {badges.filter(b => b.badge_type !== 'founding_forecaster').length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px' }}>
            {badges.filter(b => b.badge_type !== 'founding_forecaster').map((b) => {
