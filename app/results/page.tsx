        ...
        {activeMatch && (
          <div style={{ backgroundColor: '#0D2B14', border: '2px solid #2E9E5E', borderRadius: '16px', padding: '20px', marginBottom: '24px' }}>
            ...
            {loadingPredictors ? (
              ...
              <p style={{ color: '#6B7280', textAlign: 'center', padding: '20px' }}>No predictions recorded.</p>
            ) : (
                ...
                {topPredictors.map((pred, i) => {
                  ...
                  return (
                    <div key={pred.user_id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 14px', borderRadius: '10px', backgroundColor: won ? 'rgba(46,158,94,0.06)' : '#0D1F0F', border: '1px solid ' + (won ? '#1A7A4A' : '#1A3A1A') }}>
                      <div style={{ fontSize: '16px', width: '28px', textAlign: 'center' }} dangerouslySetInnerHTML={{ __html: medal }} />
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: '13px', fontWeight: 'bold' }}>{pred.username}</div>
                        <div style={{ fontSize: '11px', color: '#6B7280', marginTop: '2px' }}>
                          {getOutcomeLabel(activeMatch, pred.predicted_outcome)} &middot; {pred.confidence_pct}% confidence
                        </div>
                      </div>
            ...
            {matches.map((match) => (
              <div key={match.id} style={{ backgroundColor: '#0D2B14', border: '1px solid #1A7A4A', borderRadius: '14px', padding: '18px 20px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  <button onClick={() => loadTopPredictors(match)} style={{ flex: 1, padding: '8px', backgroundColor: '#1A7A4A', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' }}>
                    &#x1F3AF; Top Predictors
                  </button>
