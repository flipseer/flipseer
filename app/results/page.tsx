  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [matchRes, totalRes, completedRes, liveRes, upcomingRes] = await Promise.all([
        ...
        supabase.from('matches').select('*', { count: 'exact', head: true }).eq('competition', activeCompetition).gte('kickoff', activeCompetition === 'UCL 2026/27' ? '2026-09-01' : '2000-01-01'),
        supabase.from('matches').select('*', { count: 'exact', head: true }).eq('status', 'completed').eq('competition', activeCompetition).gte('kickoff', activeCompetition === 'UCL 2026/27' ? '2026-09-01' : '2000-01-01')
