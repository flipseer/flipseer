...
export default function EPLPage() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // Fetch next 8 upcoming EPL matches
  ...
  const formatKickoff = (kickoff: string) => {
    ...
    return date.toLocaleString('en-GB', { timeZone: tz, weekday: 'short', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit', hour12: true }) + (tzMap[tz] ? ' ' + tzMap[tz] : '');
  };
    ...
    <main style={{ backgroundColor: '#0D1F0F', minHeight: '100vh', fontFamily: "-apple-system,'Segoe UI',Arial,sans-serif", color: 'white', paddingBottom: 80 }}>
      ...
      <section style={{ textAlign: 'center', padding: 'clamp(48px,10vw,88px) 20px clamp(40px,8vw,72px)', borderBottom: '1px solid #1A3A1A', background: 'linear-gradient(180deg,#071408 0%,#0D1F0F 100%)', position: 'relative', overflow: 'hidden' }}>
        ...
        <p style={{ fontSize: 'clamp(15px,2.5vw,18px)', color: '#9CA3AF', lineHeight: 1.7, maxWidth: 540, margin: '0 auto 24px' }}>
          {userRank && username
            ? <>You're ranked <strong style={{ color: '#F59E0B' }}>#{userRank} globally</strong>. Every EPL prediction builds your permanent Football Reputation.</>
      ...
      <section style={{ padding: '48px 20px', borderBottom: '1px solid #1A3A1A' }}>
        <div style={{ maxWidth: 560, margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
            {[{ href: '/ucl', label: '⭐ UCL' }, { href: '/ghana', label: '🇬🇭 Ghana PL' }, { href: '/indonesia', label: '🇮🇩 Liga 1' }, { href: '/nations', label: '🌍 Nations' }, { href: '/leaderboard', label: '🏆 Leaderboard' }].map(({ href, label }) => (
