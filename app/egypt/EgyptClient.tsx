...
const EGYPTIAN_PLAYERS_IN_EUROPE = [
  'Mohamed Salah', 'Omar Marmoush', 'Mostafa Mohamed', 'Ahmed Hegazi',
  'Hamdi Fathi', 'Amr El Sulaya', 'Mohamed Elneny', 'Trézéguet',
  'Ramadan Sobhi', 'Ahmed Sayed Zizo',
];
...
export default function EgyptClient() {
    ...
    const init = async () => {
      try {
        ...
        if (allProfiles) {
          ...
          const egRank = sorted.findIndex(([code]) => code === 'EG') + 1;
          if (egRank > 0) setNationRank(egRank);
          setNationPts(nationMap['EG'] || 0);
        }
      } catch (e) {}
    };
    init();
  ...
  return (
    <main style={{ backgroundColor: '#0D1F0F', minHeight: '100vh', fontFamily: "-apple-system,'Segoe UI',Arial,sans-serif", color: 'white', paddingBottom: 80 }}>
      ...
      {/* HERO */}
      <section style={{ textAlign: 'center', padding: 'clamp(48px,10vw,88px) 20px clamp(40px,8vw,72px)', borderBottom: '1px solid #1A3A1A', background: 'linear-gradient(180deg,#0A0A1A 0%,#0D1F0F 100%)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, backgroundColor: 'rgba(206,17,38,0.1)', border: '1px solid rgba(206,17,38,0.4)', borderRadius: 999, padding: '6px 20px', marginBottom: 24 }}>
          <span style={{ width: 7, height: 7, borderRadius: '50%', backgroundColor: '#CE1121', display: 'inline-block', animation: 'pulse 1.5s infinite' }} />
          <span style={{ fontSize: 12, color: '#CE1121', fontWeight: 700, letterSpacing: '2px' }}>EGYPT · NATION BATTLE</span>
        </div>
        ...
        <h1 style={{ fontSize: 'clamp(32px,8vw,60px)', fontWeight: 900, letterSpacing: '-2px', lineHeight: 1.05, marginBottom: 16 }}>
          Egypt.<br /><span style={{ color: '#CE1121' }}>Represent the Pharaohs.</span>
        </h1>
        <p style={{ fontSize: 'clamp(15px,2.5vw,18px)', color: '#9CA3AF', lineHeight: 1.7, maxWidth: 520, margin: '0 auto 24px' }}>
          {userRank && username
            ? `You are ranked #${userRank} globally. Every EPL and UCL prediction builds your permanent Football Reputation and earns points for Egypt.`
            : 'Salah. Marmoush. Egypt produces legends. Predict every match. Represent Egypt in the global Nation Battle.'}
        </p>
        {nationRank ? (
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 12, backgroundColor: '#0D2B14', border: '1px solid #CE1121', borderRadius: 12, padding: '12px 24px', marginBottom: 24 }}>
            <span style={{ fontSize: 13, color: '#9CA3AF' }}>Egypt Nation Battle rank</span>
            <span style={{ fontSize: 24, fontWeight: 900, color: '#CE1121', fontFamily: 'Georgia, serif' }}>#{nationRank}</span>
            <span style={{ fontSize: 13, color: '#6B7280' }}>{nationPts} pts</span>
          </div>
        ) : (
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 12, backgroundColor: '#0D2B14', border: '1px solid #CE1121', borderRadius: 12, padding: '12px 24px', marginBottom: 24 }}>
            <span style={{ fontSize: 14, color: '#CE1121', fontWeight: 700 }}>Egypt is not yet on the Nation Battle — be the first to represent.</span>
          </div>
        )}
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
          <a href="/predict" style={{ backgroundColor: '#CE1121', color: 'white', border: 'none', borderRadius: 12, padding: 'clamp(12px,3vw,16px) clamp(24px,6vw,40px)', fontSize: 'clamp(14px,2.5vw,16px)', fontWeight: 700, textDecoration: 'none', boxShadow: '0 0 32px rgba(206,17,38,0.35)' }}>
            🇪🇬 Predict for Egypt →
          </a>
      ...
      <section style={{ padding: '48px 20px', borderBottom: '1px solid #1A3A1A', backgroundColor: '#050E05' }}>
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          <p style={{ fontSize: 11, color: '#CE1121', fontWeight: 700, letterSpacing: '3px', marginBottom: 20, textAlign: 'center' }}>EGYPTIAN PLAYERS IN EUROPEAN FOOTBALL</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 8 }}>
            {EGYPTIAN_PLAYERS_IN_EUROPE.map(player => (
          ...
          <p style={{ fontSize: 13, color: '#4B5563', textAlign: 'center', marginTop: 16 }}>Predict their matches. Every correct call earns points for Egypt.</p>
        </div>
      </section>
      ...
      <section style={{ padding: '48px 20px' }}>
        <div style={{ maxWidth: 560, margin: '0 auto', textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: 'clamp(22px,4vw,32px)', fontWeight: 800, letterSpacing: '-0.5px', marginBottom: 12 }}>
            Put Egypt on the map.<br /><span style={{ color: '#CE1121' }}>Build your Football Reputation.</span>
          </h2>
          <p style={{ color: '#9CA3AF', fontSize: 14, lineHeight: 1.7, marginBottom: 24 }}>
            EPL + UCL — predict every match. Permanent record. Represent Egypt in the global Nation Battle. Free forever.
          </p>
          <a href="/predict" style={{ display: 'inline-block', backgroundColor: '#CE1121', color: 'white', padding: '14px 32px', borderRadius: 10, textDecoration: 'none', fontSize: 15, fontWeight: 700, boxShadow: '0 0 24px rgba(206,17,38,0.3)', marginBottom: 16 }}>
            🇪🇬 Start Predicting — Represent Egypt →
          </a>
