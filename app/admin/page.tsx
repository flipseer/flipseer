...
export default function AdminPage() {
  ...
  // ── Auth check using env var — password never in code ──
  const handleLogin = () => {
  ...
  // ── Seed matches from API-Football ──
        ...
        {activeTab === 'settle' && (
          <div>
            <p style={{ color: '#6B7280', marginBottom: '24px', fontSize: '14px' }}>
              Enter final scores to award points. Tick "Upset" for underdog wins (+15 bonus points).
            </p>
            ...
            {matches.map(match => (
              <div key={match.id} style={{ backgroundColor: '#0D2B14', border: `1px solid ${done[match.id] ? '#2E9E5E' : match.status === 'completed' ? '#1A7A4A' : '#1A3A20'}`, borderRadius: '12px', padding: '20px', marginBottom: '12px' }}>
                <div style={{ fontSize: '11px', color: '#6B7280', marginBottom: '8px' }}>
                  {new Date(match.kickoff).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })} · {match.league} · <span style={{ color: match.status === 'completed' ? '#2E9E5E' : match.status === 'locked' ? '#F59E0B' : '#6B7280' }}>{match.status}</span>
                </div>
