...
export const metadata: Metadata = {
  title: 'Argentina Football Predictions | EPL & UCL | Flipseer',
  description: 'Argentine football fans predict EPL, UCL on Flipseer. Represent Argentina in the global Nation Battle. World Cup 2030 co-host. Free forever.',
  alternates: { canonical: 'https://flipseer.com/argentina' },
  openGraph: {
    title: 'Argentina Football Predictions | EPL & UCL | Flipseer',
    description: 'Argentine football fans predict EPL, UCL on Flipseer. Represent Argentina in the global Nation Battle. World Cup 2030 co-host. Free forever.',
    url: 'https://flipseer.com/argentina',
  },
}
...
export default function ArgentinaPage() {
  const steps = [
    ...
    { num: '3', title: 'Earn REP', desc: 'Correct predictions earn REP points for you and for Argentina in the Nation Battle.' },
  ]
  const competitions = [
    ...
    <main style={{ backgroundColor: '#0D1F0F', minHeight: '100vh', fontFamily: 'Arial, sans-serif', color: 'white', paddingBottom: '60px' }}>
      <div style={{ background: 'linear-gradient(180deg, #0D2B14 0%, #0D1F0F 100%)', padding: '48px 20px 32px', textAlign: 'center', borderBottom: '1px solid #1A3A1A' }}>
        ...
        <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(28px,6vw,44px)', marginBottom: '10px', fontWeight: 800 }}>
          Argentina Football Predictions
        </h1>
        <p style={{ color: '#9CA3AF', fontSize: '15px', maxWidth: '480px', margin: '0 auto 16px', lineHeight: 1.7 }}>
          Argentina — defending World Cup champions and 2030 co-hosts. Argentine forecasters on Flipseer predict EPL, UCL and international football — building a permanent verified record and earning REP points for Argentina in the global Nation Battle.
        </p>
        <div style={{ display: 'inline-block', backgroundColor: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.4)', borderRadius: '999px', padding: '6px 16px', marginBottom: '20px' }}>
          <span style={{ fontSize: '12px', color: '#F59E0B', fontWeight: 'bold' }}>🇦🇷 Argentina co-hosts World Cup 2030 — Build your record now</span>
        </div>
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <a href="/predict" style={{ backgroundColor: '#74ACDF', color: 'white', padding: '12px 28px', borderRadius: '10px', textDecoration: 'none', fontSize: '14px', fontWeight: 700 }}>
            ⚽ Predict EPL matches for Argentina →
          </a>
